...
from transformers import AutoModelForCausalLM, AutoModelForSeq2SeqLM, AutoTokenizer, LlamaTokenizer, pipeline, AutoConfig, BitsAndBytesConfig
import time
import numpy as np
...
def get_formated_instruction(prompt, model_name, tokenizer, task = None):
    ...
    # prompt_model_inputs = tokenizer([prompt], return_tensors="pt", add_special_tokens=False).to("cuda")
    # index_response = len(prompt_model_inputs.input_ids[0])
    
    # if index_response <= tokenizer.model_max_length:
    #     # print("可以处理",tokenizer.model_max_length)
    ...
    for i in tqdm(range(len(prompts))):
        ...
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs,
                max_new_tokens=512,  # 允许生成的最大新 token 数
                do_sample=True,       # 开启采样，增加输出的多样性
        ...
        print(f"数据已同步至缓冲区，当前总条数: {len(all_data)}")
